...
if (ARGV.size < 4)
  STDERR.print "Usage: #{$0} <experiment_path> <iters> <pop_size> <avg_runs> [config_file_path]\n"
  exit(1)
end
...
config_parameters = ["$run_path", "$sleepTime", "$maxWaitTime", "$minJobs", "$minCondorJobs", "$localCount",
  "$logEnabled", "$path_to_generator", "$evaluateFinalPolicy", "$startIter"]
...
END_OF_CONDORFILE
  ...
  inds.each do |i|
    ...
    valueFileRun = $experimentbase + "/results/run_#{gen}_i_#{i}_r_#{run}.txt";
  ...
  if (inds.size > 0)
    ...
    condorSubmitPipe.close_write
    lastLine = condorSubmitPipe.readlines.last
    puts "#{lastLine}"
    $cluster = lastLine.split("cluster").last.to_i
    #puts "#{$cluster}" 
  ...
  while(jobsRemain > $minCondorJobs)
    condorQ = open("|condor_q #{$cluster}", 'r')
    #lastLine = condorQ.readlines.last
    #lastLine =~ /([0-9]+) jobs/
    #jobsRemain = $1.to_i
    jobsRemainRead = condorQ.readlines.size-4
    if (jobsRemainRead >= 0)
  ...
  system("condor_rm cluster #{$cluster}")
  sleep $sleepTime
end
...
until(File.exists?("#{$experimentbase}/results/paramswritten_#{$startIter}.txt")) 
 print "Waiting on paramswritten_#{$startIter}.txt\n"
 sleep 1 
end

($startIter..$maxIter).each do |gen|
...
# Code to average atomic fitness evaluations.
  ...
  if($numAvgRuns == 1)
    (0..$numInds-1).each do |i|
      runFile = $experimentbase + "/results/run_#{gen}_i_#{i}_r_0.txt";
      if(File.exists?(runFile)) then
        valueFile = $experimentbase + "/results/value_#{gen}_i_#{i}.txt";
        FileUtils.mv(runFile, valueFile)
      end
    end
  else
    (0..$numInds-1).each do |i|
